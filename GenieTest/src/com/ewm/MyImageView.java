...
public class MyImageView extends ImageView {
	...
	private float initScaleRate=1.0f;
	...
	private void arithScaleRate() {
		...
		if(imageWidth==0 || imageHeight==0){
			...
			scaleWidth = ImagesBrowseActivity.linear.getWidth() / (float) imageWidth;
			scaleHeight = ImagesBrowseActivity.linear.getHeight() / (float) imageHeight;
		}
		
		if(scaleHeight>1.0f && scaleWidth>1.0f){
			scaleWidth=imageWidth / (float) image.getWidth();
			scaleHeight=imageHeight/ (float) image.getHeight();
			scaleRate = Math.max(scaleWidth, scaleHeight);
		}else{
			scaleWidth=ImagesBrowseActivity.linear.getWidth() / (float) image.getWidth();
			scaleHeight=ImagesBrowseActivity.linear.getHeight()/ (float) image.getHeight();
			if(scaleHeight>1.0f){
			...
			if(scaleRate==0){
				scaleRate=this.currentContext.getResources().getDisplayMetrics().widthPixels / (float) imageWidth;
			}
		}
			
			if(scaleRate>1.0f){
				mMaxZoom=scaleRate*2.0f;
				mMinZoom=0.5f;
//				scaleRate=1.0f;
			}else{
				mMinZoom=scaleRate/4.0f;
				mMaxZoom=2.0f;
			}
//		}
		initScaleRate=scaleRate;
	}
	...
	public void setImageBitmap(Bitmap bitmap) {
		...
		//缩放到屏幕大小
		zoomTo(scaleRate,ImagesBrowseActivity.linear.getWidth() / 2f, ImagesBrowseActivity.linear.getHeight() / 2f);
...
//		RectF rect = new RectF(0, 0, imageWidth*getScale(), imageHeight*getScale());
	...
	public void layoutToCenter()
	{
		...
		if(image==null)return;
		//正在显示的图片实际宽高
		float width = image.getWidth()*getScale();
		float height = image.getHeight()*getScale();
		
		int view_width=ImagesBrowseActivity.linear.getWidth();
		int view_height=ImagesBrowseActivity.linear.getHeight();
		if(view_width==0){
			view_width=this.currentContext.getResources().getDisplayMetrics().widthPixels;
		}
		if(view_height==0){
			//95像素为标题和下面Gallery高度
			view_height=this.currentContext.getResources().getDisplayMetrics().heightPixels-95;
		}
		
		//空白区域宽高
		float fill_width =  view_width- width;
		float fill_height = view_height - height;
