...
public class MyGallery extends Gallery {
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX,
		...
		if (view instanceof MyImageView) {
			...
			if ((int) width <= ImageDlnaShareActivity.linear.getWidth()
					&& (int) height <= ImageDlnaShareActivity.linear.getHeight())// 如果图片当前大小<屏幕大小，直接处理滑屏事件
			{
				...
				if (distanceX > 0)// 向左滑动
				{
					if (r.left > 0) {// 判断当前ImageView是否显示完全
						super.onScroll(e1, e2, distanceX, distanceY);
					} else if (right < ImageDlnaShareActivity.linear.getWidth()) {
						super.onScroll(e1, e2, distanceX, distanceY);
					} else {
				...
				{
					if (r.right < ImageDlnaShareActivity.linear.getWidth()) {
						super.onScroll(e1, e2, distanceX, distanceY);
					} else if (left > 0) {
						super.onScroll(e1, e2, distanceX, distanceY);
					} else {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (view instanceof MyImageView) {
				...
				if ((int) width <= ImageDlnaShareActivity.linear.getWidth()
						&& (int) height <= ImageDlnaShareActivity.linear.getHeight())// 如果图片当前大小<屏幕大小，判断边界
				{
					break;
				}
				float v[] = new float[9];
				...
				if (bottom < ImageDlnaShareActivity.linear.getHeight()) {
					imageView.postTranslateDur(ImageDlnaShareActivity.linear.getHeight()
							- bottom, 200f);
				}
			...
			if (view instanceof MyImageView) {
				...
				if (imageView.getScale() > imageView.getScaleRate()) {
					imageView.zoomTo(imageView.getScaleRate(),
							ImageDlnaShareActivity.linear.getWidth() / 2,
							ImageDlnaShareActivity.linear.getHeight() / 2, 200f);
					// imageView.layoutToCenter();
				} else {
					imageView.zoomTo(1.0f, ImageDlnaShareActivity.linear.getWidth() / 2,
							ImageDlnaShareActivity.linear.getHeight() / 2, 200f);
				}
